...
export async function generatePitch(idea: string): Promise<PitchDeck> {
  try {
    ...
    // If it's a JSON parse error, it might be because the model didn't return perfect JSON or it returned an error
    throw new Error(error.message || "Failed to generate pitch analysis");
  }
}
